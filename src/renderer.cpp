...
RT64Context::RT64Context(uint8_t* rdram, ultramodern::renderer::WindowHandle window_handle,
    ...
    app_config.appId = "Pilotwings64Recomp";
...
void RT64Context::send_dl(const OSTask* task) {
    ...
    if (dl_count == 0) {
        std::fprintf(stderr, "[pw64] first display list: ucode 0x%08X data 0x%08X dl 0x%08X\n",
    ...
    //
    // Bisect switch for phase 04: setting PW64_SKIP_DL isolates whether a fault
    // is inside RT64's display list processing or somewhere else entirely.
    static const bool skip_dl = std::getenv("PW64_SKIP_DL") != nullptr;
    if (!skip_dl) {
        app_->processDisplayLists(app_->core.RDRAM, physical(task->t.data_ptr), 0, true);
    }

    if (dl_count == 1) {
        std::fprintf(stderr, "[pw64] send_dl returned from the first display list\n");
        std::fflush(stderr);
    }
}
...
void RT64Context::update_screen() {
    if (valid_) {
        ...
        if (frames == 0 || frames == 60 || frames == 600) {
            std::fprintf(stderr, "[pw64] update_screen #%llu\n",

...
LONG WINAPI on_exception(EXCEPTION_POINTERS* info) {
    ...
    std::fprintf(stderr, "\n[pw64] ==== CRASH ====\n");
    std::fprintf(stderr, "[pw64] %s (0x%08lX) at %p\n", exception_name(code),
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION && record->NumberParameters >= 2) {
        const char* how = record->ExceptionInformation[0] == 0 ? "reading"
                        ...
                        : "executing";
        std::fprintf(stderr, "[pw64] while %s address 0x%llX\n", how,
    ...
    if (GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
        ...
        if (GetModuleFileNameA(module, path, sizeof(path)) != 0) {
            std::fprintf(stderr, "[pw64] in module %s (base %p, offset 0x%llX)\n",
    ...
    // the executable, so "in Pilotwings64Recomp.exe" covers ~1,300 game functions
    ...
    if (symbols_ready) {
        ...
        if (SymFromAddr(process, reinterpret_cast<DWORD64>(pc), &displacement, symbol)) {
            std::fprintf(stderr, "[pw64] in function %s + 0x%llX\n", symbol->Name,
        ...
        if (SymGetLineFromAddr64(process, reinterpret_cast<DWORD64>(pc),
            ...
            std::fprintf(stderr, "[pw64] at %s:%lu\n", line.FileName, line.LineNumber);
        }
    }
    ...
    std::fprintf(stderr, "[pw64] on thread %lu", GetCurrentThreadId());
    ...
    if (info->ContextRecord != nullptr) {
        ...
        std::fprintf(stderr, "[pw64] rip %p, rsp %p\n",
        ...
        if (readable) {
            ...
            std::fprintf(stderr, "[pw64] return addresses still on the stack:\n");
            int shown = 0;
...
void report_hang(HANDLE thread, const char* what, int seconds) {
    std::fprintf(stderr, "\n[pw64] ==== HANG ====\n");
    std::fprintf(stderr, "[pw64] %s has not returned after %d seconds\n", what, seconds);
...
extern "C" void pw64_report_lookup_miss(unsigned int addr, void* return_address) {
    static long misses = 0;
    const long index = InterlockedIncrement(&misses);

    std::fprintf(stderr, "\n[pw64] ==== LOOKUP MISS #%ld ====\n", index);
    std::fprintf(stderr, "[pw64] no function registered at 0x%08X\n", addr);
    std::fprintf(stderr, "[pw64] thread %lu\n", GetCurrentThreadId());
...
void install_crash_handler() {
    ...
    std::set_terminate([]() {
        std::fprintf(stderr, "\n[pw64] ==== UNHANDLED EXCEPTION ====\n");
        if (std::current_exception()) {
            try {
                std::rethrow_exception(std::current_exception());
            } catch (const std::exception& e) {
                std::fprintf(stderr, "[pw64] %s\n", e.what());
            } catch (...) {
                std::fprintf(stderr, "[pw64] a non-std exception\n");
            }
        } else {
            std::fprintf(stderr, "[pw64] terminate called with no active exception\n");
        }
        std::fprintf(stderr, "[pw64] =============================\n");
        std::fflush(stderr);
        std::abort();
...
//
// The one that still says something useful says it: pw64_report_lookup_miss
...
extern "C" void pw64_report_lookup_miss(unsigned int addr, void* return_address) {
    std::fprintf(stderr, "[pw64] function lookup failed at 0x%08X (caller %p)\n",
                 addr, return_address);
    std::fflush(stderr);
}

// See include/pw64/resample.h for what this is and the measurements behind it.
...
bool stats_wanted() {
    static const bool on = [] {
        const char* value = std::getenv("PW64_AUDIO_STATS");
...
void note_cost(std::chrono::steady_clock::duration spent, uint64_t frames,
    ...
    std::fprintf(stderr, "[pw64-audio] conversion: %.2f%% of one core"
                         " (%.0f frames a second)\n",
...
void configure(uint32_t src_rate, uint32_t dst_rate) {
    ...
    if (src_rate == 0 || dst_rate == 0 || src_rate == dst_rate) {
        g_passthrough = true;
        std::fprintf(stderr, "[pw64] no resampling needed (%u Hz both sides)\n", src_rate);
        std::fflush(stderr);
        return;
    }
    g_passthrough = false;
    ...
    std::fprintf(stderr, "[pw64] resampling %u -> %u Hz in the port"

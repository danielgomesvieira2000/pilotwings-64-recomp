// See include/pw64/audiodiag.h for what this measures and why.
...
void report(clock::time_point now) {
    ...
    std::fprintf(stderr, "[pw64-audio] %.2f s at %u Hz: %.0f buffers/s of %.0f frames (%u-%u)\n",
                 seconds, g_rate, double(g_buffers) / seconds, avg_buffer,
                 g_buffer_min, g_buffer_max);
    std::fprintf(stderr, "[pw64-audio]   queue %u / %llu / %u frames (%.1f / %.1f / %.1f ms),"
    ...
    if (g_have_depth && g_depth_min >= g_period_frames) {
        std::fprintf(stderr, "[pw64-audio]   produced %llu frames, the device wanted %.0f:"
    ...
    std::fprintf(stderr, "[pw64-audio]   peak amplitude %d of 32767\n", g_peak);
    std::fflush(stderr);
}
...
void open_wav(uint32_t rate) {
    ...
    char name[64];
    std::snprintf(name, sizeof(name), "pw64-audio-%d-%uHz.wav", ++g_wav_index, rate);
    const std::string path = (recomp::get_config_path() / name).string();
    g_wav = std::fopen(path.c_str(), "wb");
    if (g_wav == nullptr) {
        std::fprintf(stderr, "[pw64-audio] could not open %s for the dump\n", path.c_str());
        std::fflush(stderr);
        return;
    }
    write_wav_header(rate, 0);
    std::fprintf(stderr, "[pw64-audio] dumping what is handed to SDL to %s\n", path.c_str());
    std::fflush(stderr);
}
...
bool stats_enabled() {
    static const bool on = env_set("PW64_AUDIO_STATS");
    return on;
}

bool dump_enabled() {
    static const bool on = env_set("PW64_AUDIO_DUMP");
    return on;
}
...
void device_opened(uint32_t asked_rate, uint32_t asked_period_frames,
    ...
    if (stats_enabled()) {
        ...
        if (device_rate == 0) {
            std::fprintf(stderr, "[pw64-audio] device: asked %u Hz, %u-frame buffer (%.1f ms);"

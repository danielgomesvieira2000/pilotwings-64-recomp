// Pilotwings 64: Recompiled -- entry point.
//
// Registers the game with librecomp, supplies the platform callbacks
// ultramodern needs, and starts the runtime. The recompiled game code is reached
// through recomp_entrypoint, which N64Recomp generated.
//
// --identify checks a dump against the pinned target before anything else is
// attempted.
...
#ifndef PW64_VERSION
#   define PW64_VERSION "unknown"
#   define PW64_VERSION_MAJOR 0
#   define PW64_VERSION_MINOR 0
#   define PW64_VERSION_PATCH 0
#endif
...
//
// On Windows this is %LOCALAPPDATA%\Pilotwings64Recomp, the same folder RT64
// already keeps its own files in, so a player has one place to look. On macOS
// it is ~/Library/Application Support/Pilotwings64Recomp, which is where a Mac
// application is expected to put this and where Migration Assistant will carry
// it. On Linux and the rest it is $XDG_DATA_HOME/Pilotwings64Recomp, or
// ~/.local/share/Pilotwings64Recomp.
...
std::filesystem::path settings_directory() {
    ...
    return root / "Pilotwings64Recomp";
}
...
void on_init(uint8_t* rdram, recomp_context* ctx) {
    ...
    std::printf("[pw64] runtime initialised; entering recomp_entrypoint\n");
    std::printf("[pw64] rdram base = %p\n", static_cast<void*>(rdram));
    std::fflush(stdout);

    // The state watcher needs the RDRAM base, and this is the first place the
    // project has it.
    pw64::set_rdram_base(rdram);

    // librecomp has just registered the boot megabyte as one contiguous load,
...
void on_thread_create(uint8_t* rdram, recomp_context* ctx) {
    (void)rdram;
    static int count = 0;
    std::printf("[pw64] game thread %d created (entry 0x%08X)\n",
                ++count, static_cast<uint32_t>(ctx->r4));
    std::fflush(stdout);
}

// XXH3-64 of the pinned dump, which is the identity librecomp checks. Its sha1
// and header CRC are recorded in include/pw64/rom.h; this is the same dump.
...
void print_usage(const char* argv0) {
    std::printf(
        ...
        "Pilotwings 64 (USA) dump.\n",
        argv0, argv0, argv0);
}

void print_version() {
    std::printf("Pilotwings 64: Recompiled %s\n", PW64_VERSION);
    std::printf("  recompiled game code : %s\n",
#if PW64_WITH_RECOMPILED
        "linked");
#else
        "not built (configure with -DPW64_WITH_RECOMPILED=ON)");
#endif
    std::printf("  runtime + RT64       : %s\n",
#if PW64_WITH_RUNTIME
        "linked");
#else
        "not built (configure with -DPW64_WITH_RUNTIME=ON)");
#endif
#if PW64_WITH_RECOMPILED
    std::printf("  code sections        : %zu\n", pw64::code_section_count());
#endif
}
...
int identify(const char* path_arg) {
    ...
    if (!pw64::read_header(path, header, error)) {
        std::fprintf(stderr, "error: %s\n", error.c_str());
        return 1;
    }

    std::printf("%-16s %s\n", "file",     path);
    std::printf("%-16s %s\n", "format",   pw64::to_string(header.format).c_str());
    ...
    if (pw64::verify(header, problems)) {
...
#if PW64_WITH_RUNTIME && PW64_WITH_RECOMPILED
...
int run(int argc, char** argv, const char* rom_arg) {
    ...
    if (rom_path != nullptr) {
        pw64::RomHeader header;
        std::string error;
        if (!pw64::read_header(rom_path, header, error)) {
            std::fprintf(stderr, "error: %s\n", error.c_str());
            return 1;
        }
        std::vector<std::string> problems;
    ...
    {
        const std::filesystem::path settings = pw64::settings_directory();
        std::error_code ec;
        std::filesystem::create_directories(settings, ec);
        recomp::register_config_path(settings);
        std::fprintf(stderr, "[pw64] settings directory: %s\n", settings.string().c_str());
    }
    ...
    game.display_name = pw64::kDisplayName;
    game.game_id = pw64::kGameId;
    game.mod_game_id = pw64::kModGameId;
    // Pilotwings 64 keeps its save files on a 4 Kbit EEPROM (osEepromLongRead
    // and osEepromLongWrite in the decompilation's src/kernel/system.c).
    game.save_type = recomp::SaveType::Eep4k;
    game.is_enabled = true;
    game.entrypoint_address = static_cast<gpr>(static_cast<int32_t>(pw64::kEntrypoint));
    game.entrypoint = recomp_entrypoint;
    game.on_init_callback = on_init;
    game.thread_create_callback = on_thread_create;

#if PW64_WITH_FRONTEND
    pw64::frontend::publish_game(game);
#endif
    ...
    if (rom_path != nullptr) {
        const recomp::RomValidationError rom_error =
            recomp::select_rom(std::filesystem::path{rom_path}, std::u8string{pw64::kGameId});
    ...
    config.project_version =
        recomp::Version{ PW64_VERSION_MAJOR, PW64_VERSION_MINOR, PW64_VERSION_PATCH, "" };
    config.rsp_callbacks = pw64::rsp_callbacks();
#if PW64_WITH_FRONTEND
    ...
    config.renderer_callbacks = pw64::frontend::renderer_callbacks();
#else
    config.renderer_callbacks = pw64::renderer_callbacks();
#endif
    config.audio_callbacks = pw64::audio_callbacks();
    config.input_callbacks = pw64::input_callbacks();
    config.gfx_callbacks = pw64::gfx_callbacks();
    config.events_callbacks = pw64::events_callbacks();
    config.error_handling_callbacks = pw64::error_handling_callbacks();
    config.threads_callbacks = pw64::threads_callbacks();
    ...
    if (rom_path != nullptr) {
        recomp::start_game(std::u8string{pw64::kGameId}, std::string{});
    }
#else
    recomp::start_game(std::u8string{pw64::kGameId}, std::string{});
#endif
...
#endif  // PW64_WITH_RUNTIME && PW64_WITH_RECOMPILED

}  // namespace

int main(int argc, char** argv) {
    pw64::install_crash_handler();
    ...
    // the working directory. Started as build-fe\Pilotwings64Recomp.exe from the
    ...
    // as a relative path still resolves against it.
    {
        ...
        const std::filesystem::path exe_dir = pw64::executable_directory(argv[0]);
        if (!exe_dir.empty()) {
            std::filesystem::current_path(exe_dir, ec);
            if (ec) {
                std::fprintf(stderr, "[pw64] could not change to %s: %s\n",
    ...
    // released, and everything the port prints goes to pw64.log in the settings
    ...
    // old handles are still valid to close.
    {
        ...
        if (GetConsoleWindow() != nullptr && GetConsoleProcessList(owners, 2) == 1) {
            const std::filesystem::path log = pw64::settings_directory() / "pw64.log";
            std::error_code ec;
            ...
            if (redirected) {
                std::fprintf(stderr, "[pw64] output goes to this file when the game is started"
                                     " without a terminal\n");
            }
        }
    }
...
#if PW64_WITH_FRONTEND && PW64_WITH_RUNTIME && PW64_WITH_RECOMPILED
    ...
    std::fprintf(stderr,
                 ...
                 "  -DPW64_WITH_RUNTIME=ON -DPW64_WITH_RECOMPILED=ON\n");
    return 1;
#endif
}

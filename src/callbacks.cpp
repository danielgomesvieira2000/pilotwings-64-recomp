...
#include "pw64/audiodiag.h"
#include "pw64/crash_handler.h"
#include "pw64/resample.h"
    ...
    while (SDL_PollEvent(&event)) {
        switch (event.type) {
            case SDL_QUIT:
                ...
                std::fprintf(stderr, "[pw64] SDL_QUIT received; asking the runtime to quit\n");
                std::fflush(stderr);
                ultramodern::quit();
                break;
...
float axis_to_n64(Sint16 value) {
    // Flight is analogue throughout, so the stick is rescaled past the deadzone
    // rather than cut off at it.
...
bool get_input(int controller_num, uint16_t* buttons, float* x, float* y) {
    ...
    if (!pw64::frontend::capturing_input()) {
        ...
        // src/testdrive.cpp), so it is scaled to match.
        {
            ...
            pw64::input_script_state(controller_num, &scripted_buttons, &scripted_x, &scripted_y);
    ...
    *y = stick_y;
    ...
    if (keys != nullptr) {
        ...
        // The C buttons work the camera and, on some vehicles, the throttle, so
        // they need to be reachable without a pad. I/J/K/L keeps them under the
        // right hand while the left flies with the arrow keys.
    ...
    float scripted_x = 0.0f;
    float scripted_y = 0.0f;
    pw64::input_script_state(&scripted_buttons, &scripted_x, &scripted_y);
    pressed |= scripted_buttons;
...
// The runtime's rumble callback. Pilotwings 64 predates the Rumble Pak and never
// reaches it, but it is wired through the frontend so that one path owns the
// motor if a mod ever asks for it.
...
// It does that badly (see include/pw64/resample.h), so the device is opened at
...
// PW64_AUDIO_STATS at a 1024-frame period, the queue averaged 469 frames against
...
// shorter block was cleaner. The port converts now (see pw64/resample.h), so
...
uint32_t queue_headroom_ms() {
    static const uint32_t value = [] {
        const char* env = std::getenv("PW64_AUDIO_HEADROOM_MS");
...
int device_period_frames() {
    static const int value = [] {
        const char* env = std::getenv("PW64_AUDIO_PERIOD");
...
bool resample_in_port() {
    static const bool on = std::getenv("PW64_AUDIO_NO_RESAMPLE") == nullptr;
    return on;
}
...
bool open_audio_device() {
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0) {
        std::fprintf(stderr, "[pw64] SDL_InitSubSystem(AUDIO) failed: %s\n", SDL_GetError());
        return false;
    }
    ...
    if (g_audio_device == 0) {
        std::fprintf(stderr, "[pw64] SDL_OpenAudioDevice failed: %s\n", SDL_GetError());
        return false;
    }
    ...
    std::fprintf(stderr, "[pw64] audio device open at %d Hz, %d channels,"
                         " %d-frame period, holding %u of the game's frames (%u ms) in hand\n",
                 have.freq, have.channels, have.samples,
                 g_queue_headroom_frames, queue_headroom_ms());
    std::fflush(stderr);

    pw64::resample::configure(g_audio_frequency, g_device_frequency);
    ...
    pw64::audiodiag::device_opened(g_device_frequency,
...
void queue_samples(int16_t* audio_data, size_t sample_count) {
    ...
    if (!announced_first) {
        ...
        std::fprintf(stderr, "[pw64] first audio buffer queued: %zu frames at %u Hz\n",
    ...
    // device happens to run at the game's rate. See include/pw64/resample.h.
    static std::vector<int16_t> converted;
    pw64::resample::process(unswapped.data(), sample_count, converted);
    ...
    if (!announced) {
        for (size_t i = 0; i < sample_count; ++i) {
            if (unswapped[i] != 0) {
                announced = true;
                std::fprintf(stderr, "[pw64] audio is audible (first non-silent buffer"
                                     " after %zu frames)\n", sample_count / 2);
                std::fflush(stderr);
                break;
            }
...
size_t get_frames_remaining() {
    ...
    // PW64_AUDIO_STATS is set.
    pw64::audiodiag::polled(static_cast<uint32_t>(device_frames));
    ...
    const size_t frames =
        static_cast<size_t>(pw64::resample::to_source_frames(uint64_t(device_frames)));
...
constexpr uint32_t kAspMainTextStart = 0x80247E60;
...
RspExitReason rsp_unknown_ucode(uint8_t* rdram, uint32_t ucode_addr) {
    ...
    if (!reported) {
        reported = true;
        std::fprintf(stderr, "[pw64] unrecognised RSP microcode at 0x%08X; "
...
// click Wave Race 64's port could characterise before it could place it.
// Pilotwings 64 runs the same microcode, so the same defence applies. Copying
// the list before the run costs a memcpy of a few KB per frame and makes the
// outcome independent of how late the task runs. Where the copy lives is
// explained at kCommandListScratch.
//
...
void bisect_audio_task(uint8_t* rdram, uint32_t ucode_addr) {
    ...
    for (uint32_t n = 1; n <= total && what == nullptr; ++n) {
        ...
        if (!top_reported && std::memcmp(&dmem[0xFC0], &copy, sizeof(OSTask)) != 0) {
            ...
            std::fprintf(stderr, "[pw64-audio] (top of DMEM 0x%03X-0x%03X first written after command %u)\n",
        ...
        if (std::memcmp(&dmem[0x10], &pristine[0x10], 0x20) != 0) {
            ...
            std::fprintf(stderr, "[pw64-audio] bytes changed in 0x000-0x040:");
            for (uint32_t i = 0; i < 0x40; ++i) {
    ...
    if (what == nullptr) {
        std::fprintf(stderr, "[pw64-audio] every prefix ran clean on re-run: the failure depends on"
                             " state the first run changed, not on the command list alone\n");
        std::fflush(stderr);
        return;
    }
    std::fprintf(stderr, "[pw64-audio] first prefix to damage %s is %u commands:"
    ...
    for (uint32_t i = from; i < bad + 2 && i < total; ++i) {
        ...
        std::fprintf(stderr, "[pw64-audio]  %s%3u %-10s f=%02X a=%04X  b=%04X c=%04X   (%08X %08X)\n",
...
// Pilotwings 64 reads osMemSize and grows its heap to whatever the runtime
// reports (uvLevelInit in the decompilation), and the runtime reports 8 MB. The
// runtime's own structures start at 0x80800000 (PI handles, then the patches
// from 0x80801000) and mods load from 0x81000000, so the megabyte below that is
// free for the port.
...
RspExitReason asp_main_watched(uint8_t* rdram, uint32_t ucode_addr) {
    ...
    pw64::watch_for_hang("the audio microcode", 5);
...

    {
        ...
        if (std::memcmp(before.data(), rdram + original_base, g_audio_task_size) != 0) {
            ++changed_runs;
            if (changed_runs <= 3) {
                std::fprintf(stderr, "[pw64] the game rewrote an audio command list while its task was running"
    ...
    if (reason != RspExitReason::Broke) {
        ...
        if (dropped <= 40 || dropped % 100 == 0) {
            std::fprintf(stderr, "[pw64] audio frame dropped: the microcode did not reach"
                                 " its break (%llu so far). See src/callbacks.cpp.\n",
                         static_cast<unsigned long long>(dropped));
            std::fflush(stderr);
        }
        if (dropped <= 6) {
            std::fprintf(stderr, "[pw64-audio] task failed with %s: %u commands at 0x%08X\n",
...
RspUcodeFunc* get_rsp_microcode(const OSTask* task) {
    ...
    if (++others <= 5) {
        std::fprintf(stderr, "[pw64] non-audio RSP task: type %u ucode 0x%08X\n",
...
void message_box(const char* msg) {
    ...
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, "Pilotwings 64: Recompiled", msg, g_window);
}
...
std::string get_game_thread_name(const OSThread* t) {
    ...
    // several platforms.
    char buf[16];
    std::snprintf(buf, sizeof(buf), "pw64_%d", t != nullptr ? t->id : -1);
...
ultramodern::renderer::WindowHandle create_window(ultramodern::gfx_callbacks_t::gfx_data_t) {
    ...
    if (fullscreen && SDL_GetDisplayBounds(0, &display) == 0) {
        ...
        std::fprintf(stderr, "[pw64] window %dx%d fullscreen (the display's own size)\n",
                     width, height);
        std::fflush(stderr);
    }
    else {
        ...
        if (SDL_GetDisplayUsableBounds(0, &usable) == 0) {
            ...
            width = 320 * scale;
            height = 240 * scale;
            std::fprintf(stderr, "[pw64] window %dx%d (%dx upscale; display has %dx%d usable)\n",
                         width, height, scale, usable.w, usable.h);
            std::fflush(stderr);
        }
    }

    g_window = SDL_CreateWindow("Pilotwings 64: Recompiled",
    ...
    pw64::frontend::publish_window(g_window);
#endif
...
void update_gfx(ultramodern::gfx_callbacks_t::gfx_data_t) {
    ...
    if (ticks == 0 || ticks == 1000 || ticks == 10000) {
        std::fprintf(stderr, "[pw64] main loop tick %llu\n",
                     static_cast<unsigned long long>(ticks));
        std::fflush(stderr);
    }
    ++ticks;
    poll_input();
    pw64::poll_game_state();
...
void set_audio_volume(double percent) {
    ...
    std::fprintf(stderr, "[pw64] main volume: %d%%%s\n", clamped,
...
void shutdown_platform() {
    std::fprintf(stderr, "[pw64] shutting down: controller\n");
    std::fflush(stderr);
    if (g_controller != nullptr) {
        SDL_GameControllerClose(g_controller);
        g_controller = nullptr;
    }
    std::fprintf(stderr, "[pw64] shutting down: window\n");
    std::fflush(stderr);
    if (g_window != nullptr) {
        SDL_DestroyWindow(g_window);
        g_window = nullptr;
    }
    std::fprintf(stderr, "[pw64] shutting down: SDL\n");
    std::fflush(stderr);
    SDL_Quit();
    std::fprintf(stderr, "[pw64] shutting down: done\n");
    std::fflush(stderr);
}
